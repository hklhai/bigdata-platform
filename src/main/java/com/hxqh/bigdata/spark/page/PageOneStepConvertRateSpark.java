...
import com.hxqh.bigdata.dao.IPageSplitConvertRateDAO;
...
public class PageOneStepConvertRateSpark {
    ...
    public static void main(String[] args) {
        ...
        // 脱离了session，生成的页面访问切片就没有意义了
        // 举例，比如用户A，访问了页面3和页面5
        // 用于B，访问了页面4和页面6
        // 漏了一个前提，使用者指定的页面流筛选条件，比如页面3->页面4->页面7
        // 不能将页面3->页面4，串起来，作为一个页面切片，来进行统计呢
        // 所以页面切片的生成，肯定是要基于用户session粒度的
        JavaPairRDD<String, Row> sessionid2actionRDD = getSessionid2actionRDD(actionRDDByDateRange);

        // cache等价于persist(StorageLevel.MEMORY_ONLY)
        sessionid2actionRDD = sessionid2actionRDD.cache();

        // 对<sessionid,访问行为> RDD，做groupByKey操作
        // 需要每个session对应的访问行为数据，才能够去生成切片
        JavaPairRDD<String, Iterable<Row>> sessionid2actionsRDD = sessionid2actionRDD.groupByKey();

        // 核心，每个session的单跳页面切片的生成，以及页面流的匹配，算法
     ...
     *
     * @param actionRDDByDateRange 用户访问行为RDD
     * @return <sessionid,用户访问行为>格式的数据
     */
    private static JavaPairRDD<String, Row> getSessionid2actionRDD(JavaRDD<Row> actionRDDByDateRange) {
    ...
    private static JavaPairRDD<String, Integer> generateAndMatchPageSplit(JavaSparkContext sc, JavaPairRDD<String, Iterable<Row>> sessionid2actionsRDD, JSONObject taskParam) {
        ...
        return sessionid2actionsRDD.flatMapToPair(new PairFlatMapFunction<Tuple2<String, Iterable<Row>>, String, Integer>() {
            ...
            public Iterable<Tuple2<String, Integer>> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                ...
                // session的访问行为，默认情况下是乱序的
                // 比如说，正常情况下，我们希望拿到的数据，是按照时间顺序排序的
                // 但是问题是，默认是不排序的
                // 所以，先对session的访问行为数据按照时间进行排序
    ...
    private static Map<String, Double> computePageSplitConvertRate(JSONObject taskParam, Map<String, Object> pageSplitPvMap, Long startPagePv) {
        Map<String, Double> convertRateMap = new HashMap<>(20);
        // 用户指定的页面切割流
        ...
        for (int i = 1; i < targetPages.length; i++) {
            String targetPageSplit = targetPages[i - 1] + "_" + targetPages[i];
            Object o = pageSplitPvMap.get(targetPageSplit);
            Long targetPageSplitPv = null;
            if (o != null) {
                targetPageSplitPv = Long.valueOf(String.valueOf(o));
            } else {
                targetPageSplitPv = 1L;
            }
            ...
            if (i == 1) {
                ...
                System.out.println(targetPageSplitPv);
                if (startPagePv == 0) {
                    startPagePv = 1L;
                }
                System.out.println(startPagePv);
