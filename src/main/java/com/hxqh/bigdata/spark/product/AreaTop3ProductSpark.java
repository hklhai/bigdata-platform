...
import com.hxqh.bigdata.dao.IAreaTop3ProductDAO;
...
//        // reduce join转换为map join：spark.sql.autoBroadcastJoinThreshold（默认是10485760 ）
//        sqlContext.setConf("spark.sql.autoBroadcastJoinThreshold", "20971520");
        ...
        // 3. 查询获取任务参数
        long taskId = ParamUtils.getTaskIdFromArgs(args, Constants.SPARK_LOCAL_TASKID_AREA);
        ...
        List<Row> rowList = areaTop3ProductRDD.collect();
        System.out.println("rowList: " + rowList.size());
        persistAreaTop3Product(taskId, rowList);
